...
class Builder:
    ...
    def make_video_loop(self, src: Path, dst: Path, clip_no: int):
        ...
        # Flow/Kling clips may already contain a clean start-to-end loop.
        # Adding another crossfade to them creates a visible double image every
        # few seconds, so repeat the unit unchanged in that case.
...
def parse_args(argv=None):
    ...
    p.add_argument("--already-looped",action="store_true",help="Клипы уже являются бесшовными loop; не накладывать дополнительный переход внутри клипа")
